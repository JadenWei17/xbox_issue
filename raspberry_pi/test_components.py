...
class UDPValidationTests(unittest.TestCase):
    def test_valid_packet(self) -> None:
        packet = json.dumps(
            {"x": 0.2, "y": -1.0, "left_stick_pressed": False}
        ).encode()
    ...
    def test_invalid_packets(self) -> None:
        packets = [
            b"{}",
            b"not-json",
            json.dumps({"x": 2, "y": 0, "left_stick_pressed": False}).encode(),
            json.dumps({"x": True, "y": 0, "left_stick_pressed": False}).encode(),
            json.dumps({"x": 0, "y": 0, "left_stick_pressed": 0}).encode(),
        ]
