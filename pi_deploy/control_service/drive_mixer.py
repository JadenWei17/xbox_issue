...
def apply_minimum_wheel_speed(
    left: int, right: int, minimum_speed: int, maximum_speed: int
) -> tuple[int, int]:
    """Raise each non-zero manual wheel command to the effective PWM floor."""
    floor = max(0, min(abs(minimum_speed), abs(maximum_speed)))
    limit = abs(maximum_speed)

    def apply(value: int) -> int:
        value = max(-limit, min(limit, value))
        if value == 0:
            return 0
        return max(floor, abs(value)) * (1 if value > 0 else -1)
